...
let
  cfg = config.jka.services.glance;
in
{
  options.jka.services.glance = {
    enable = lib.mkEnableOption "Glance dashboard";

    domain = lib.mkOption {
      type = lib.types.str;
      default = "dash.jka.one";
      description = "Primary domain for the Glance dashboard.";
    };

    serverAliases = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ "start.jka.one" ];
    ...
    port = lib.mkOption {
      type = lib.types.port;
      default = 5346;
      description = "Internal port for Glance.";
    };
  };
